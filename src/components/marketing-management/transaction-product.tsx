...
import ProductTransactionForm from "@/components/formModal/product-transaction-form";
import {
...
} from "@/services/transactionproduct.service";
import Swal from "sweetalert2";

type FilterStatus = "semua" | "Aktif" | "Tidak Aktif";

export default function TransactionProductPage() {
  const [form, setForm] = useState<Partial<ProductTransaction>>({
    status: true,
  });
  ...
  const { data, isLoading, refetch } = useGetProductTransactionsQuery({
    page: 1,
    paginate: 1000, // ambil semua untuk di-filter lokal
    search,
  });

  const [createProductTransaction] = useCreateProductTransactionMutation();
  const [updateProductTransaction] = useUpdateProductTransactionMutation();
  const [deleteProductTransaction] = useDeleteProductTransactionMutation();
  ...
  const handleSubmit = async () => {
    try {
      if (editingId !== null) {
        ...
        await createProductTransaction(form).unwrap();
  ...
  const handleEdit = (data: ProductTransaction) => {
    setForm(data);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteProductTransaction(id).unwrap();
        refetch();
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (filterStatus === "Aktif") {
      trx = trx.filter((t) => t.status === true);
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">Data Transaksi Produk</h1>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted text-left">
              <tr>
                ...
                <th className="px-4 py-2 whitespace-nowrap">Tgl Transaksi</th>
